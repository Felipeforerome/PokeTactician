// <auto-generated />
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using PokeTactician.Models;

#nullable disable

namespace PokeTactician.Migrations
{
    [DbContext(typeof(PokemonContext))]
    [Migration("20241212213932_TypeEffectiveness")]
    partial class TypeEffectiveness
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.0")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("Game", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Games");
                });

            modelBuilder.Entity("GamePokemon", b =>
                {
                    b.Property<int>("GamesId")
                        .HasColumnType("integer");

                    b.Property<int>("PokemonId")
                        .HasColumnType("integer");

                    b.HasKey("GamesId", "PokemonId");

                    b.HasIndex("PokemonId");

                    b.ToTable("GamePokemon");
                });

            modelBuilder.Entity("Move", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<float>("Accuracy")
                        .HasColumnType("real");

                    b.Property<string>("Damage_Class")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("PP")
                        .HasColumnType("integer");

                    b.Property<int>("Power")
                        .HasColumnType("integer");

                    b.Property<int>("Priority")
                        .HasColumnType("integer");

                    b.Property<int>("TypeId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("TypeId");

                    b.ToTable("Moves");
                });

            modelBuilder.Entity("MovePokemon", b =>
                {
                    b.Property<int>("KnowableMovesId")
                        .HasColumnType("integer");

                    b.Property<int>("PokemonId")
                        .HasColumnType("integer");

                    b.HasKey("KnowableMovesId", "PokemonId");

                    b.HasIndex("PokemonId");

                    b.ToTable("MovePokemon");
                });

            modelBuilder.Entity("Pokemon", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("Att")
                        .HasColumnType("integer");

                    b.Property<bool>("BattleOnly")
                        .HasColumnType("boolean");

                    b.Property<int>("Deff")
                        .HasColumnType("integer");

                    b.Property<int>("Hp")
                        .HasColumnType("integer");

                    b.Property<bool>("Legendary")
                        .HasColumnType("boolean");

                    b.Property<bool>("Mega")
                        .HasColumnType("boolean");

                    b.Property<bool>("Mythical")
                        .HasColumnType("boolean");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("SpAtt")
                        .HasColumnType("integer");

                    b.Property<int>("SpDeff")
                        .HasColumnType("integer");

                    b.Property<int>("Spe")
                        .HasColumnType("integer");

                    b.Property<int>("Type1Id")
                        .HasColumnType("integer");

                    b.Property<int?>("Type2Id")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("Type1Id");

                    b.HasIndex("Type2Id");

                    b.ToTable("Pokemons");
                });

            modelBuilder.Entity("PokemonType", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Types");
                });

            modelBuilder.Entity("TypeEffectiveness", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("AttackingTypeId")
                        .HasColumnType("integer");

                    b.Property<int>("DefendingTypeId")
                        .HasColumnType("integer");

                    b.Property<double>("Effectiveness")
                        .HasColumnType("double precision");

                    b.HasKey("Id");

                    b.HasIndex("AttackingTypeId");

                    b.HasIndex("DefendingTypeId");

                    b.ToTable("TypeEffectivenesses");
                });

            modelBuilder.Entity("GamePokemon", b =>
                {
                    b.HasOne("Game", null)
                        .WithMany()
                        .HasForeignKey("GamesId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Pokemon", null)
                        .WithMany()
                        .HasForeignKey("PokemonId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Move", b =>
                {
                    b.HasOne("PokemonType", "Type")
                        .WithMany("Moves")
                        .HasForeignKey("TypeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Type");
                });

            modelBuilder.Entity("MovePokemon", b =>
                {
                    b.HasOne("Move", null)
                        .WithMany()
                        .HasForeignKey("KnowableMovesId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Pokemon", null)
                        .WithMany()
                        .HasForeignKey("PokemonId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Pokemon", b =>
                {
                    b.HasOne("PokemonType", "Type1")
                        .WithMany("PrimaryTypePokemons")
                        .HasForeignKey("Type1Id")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("PokemonType", "Type2")
                        .WithMany("SecondaryTypePokemons")
                        .HasForeignKey("Type2Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Type1");

                    b.Navigation("Type2");
                });

            modelBuilder.Entity("TypeEffectiveness", b =>
                {
                    b.HasOne("PokemonType", "AttackingType")
                        .WithMany("Attacking")
                        .HasForeignKey("AttackingTypeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("PokemonType", "DefendingType")
                        .WithMany("Defending")
                        .HasForeignKey("DefendingTypeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("AttackingType");

                    b.Navigation("DefendingType");
                });

            modelBuilder.Entity("PokemonType", b =>
                {
                    b.Navigation("Attacking");

                    b.Navigation("Defending");

                    b.Navigation("Moves");

                    b.Navigation("PrimaryTypePokemons");

                    b.Navigation("SecondaryTypePokemons");
                });
#pragma warning restore 612, 618
        }
    }
}
